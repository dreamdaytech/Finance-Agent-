...
import { Plus, Trash2, ArrowLeft, Loader2, Download, Share2, User, Package } from 'lucide-react';
...
export default function DocumentEdit() {
  ...
  const [originalRelatedDocumentId, setOriginalRelatedDocumentId] = useState('');
    ...
    async function fetchDocument() {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setDate(docDate.toISOString().split('T')[0]);
          ...
          setCustomCustomerDetails(data.customCustomerDetails || []);
          setCustomTotals(data.customTotals || []);
          setRelatedDocumentId(data.relatedDocumentId || '');
          setRelatedDocumentNumber(data.relatedDocumentNumber || '');
          setOriginalRelatedDocumentId(data.relatedDocumentId || '');
          ...
          if (data.recurringEndDate) {
            const endDate = data.recurringEndDate instanceof Date ? data.recurringEndDate : (data.recurringEndDate as any).toDate();
  ...
  const handleSave = async (action?: 'download' | 'share') => {
    ...
    try {
      ...
      if (isRecurring) {
        ...
        docData.isRecurring = false;
        docData.recurringFrequency = null;
        docData.nextRecurringDate = null;
        docData.recurringEndDate = null;
      }

      await updateDoc(docRef, docData);
      
      // Handle bidirectional linking updates
      if (originalRelatedDocumentId !== relatedDocumentId) {
        // Remove link from old related document
        if (originalRelatedDocumentId) {
          try {
            const oldRelatedDocRef = doc(db, 'documents', originalRelatedDocumentId);
            await updateDoc(oldRelatedDocRef, {
              relatedDocumentId: '',
              relatedDocumentNumber: '',
              updatedAt: serverTimestamp()
            });
          } catch (err) {
            console.error('Failed to remove link from old related document:', err);
          }
        }
        
        // Add link to new related document
        if (relatedDocumentId) {
          try {
            const newRelatedDocRef = doc(db, 'documents', relatedDocumentId);
            await updateDoc(newRelatedDocRef, {
              relatedDocumentId: id,
              relatedDocumentNumber: documentNumber,
              updatedAt: serverTimestamp()
            });
          } catch (err) {
            console.error('Failed to add link to new related document:', err);
          }
        }
      }
      ...
      if (action === 'download') {
        await generatePDF({ ...docData, id } as any, profile);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background border-t md:relative md:bg-transparent md:border-0 md:p-0 flex flex-col sm:flex-row justify-end gap-2">
        <Button variant="outline" size="lg" className="w-full sm:w-auto" onClick={() => handleSave('download')} disabled={saving}>
          <Download className="mr-2 h-4 w-4" /> Save & Download
        </Button>
        <Button variant="outline" size="lg" className="w-full sm:w-auto" onClick={() => handleSave('share')} disabled={saving}>
